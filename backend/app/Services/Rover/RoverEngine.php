...
{
    public function run(int $x, int $y, string $direction, string $commands): array
    {
        ...
        foreach ($commands_array as $c) {                                       // Bucle que tracta cada caracter del array
            if ($c === 'L') {                                                   // En cas L --> girar a esquerra
                ...
                [$x, $y] = $this->finsEndavant($x, $y, $dir);                   // Cridem directament a una funcio, aquesta funcio ens retorna directament la nova posició X i Y
            } else {
    ...
    private function finsEndavant(int $x, int $y, string $dir): array
    {
